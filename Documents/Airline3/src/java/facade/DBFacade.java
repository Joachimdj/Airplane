...
public class DBFacade implements DBInterface{
    ...
    public List<Flight> getAllFlights(String startAirport, Long departuredate) {
        List<Flight> allFlights = new ArrayList();
          em.getTransaction().begin();
        Query query = em.createQuery("SELECT e FROM Flight e WHERE e.takeOffDate='"+departuredate+"'  AND e.depature='"+startAirport.toLowerCase()+"' ");
  ...
  public List<Flight> getAllFlights2(String startAirport,String endAirport, String date) {
        List<Flight> allFlights = new ArrayList();
            em.getTransaction().begin();
        Query query = em.createQuery("SELECT e FROM Flight e WHERE e.destination='"+endAirport.toLowerCase()+"' AND e.depature='"+startAirport.toLowerCase()+"' AND e.takeOffDate>='"+date+"'");
        allFlights = query.getResultList();
  ...
  @Override
    public boolean CreateReservation(Passenger[] passengerObjects, long flightID) {
        ...
        Passenger p = new Passenger(); 
         p.setFirstName(passengerObject.getFirstName());
         p.setLastName(passengerObject.getLastName());
